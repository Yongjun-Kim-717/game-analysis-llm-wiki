...
This repository includes sample game pages so a first-time user can inspect the finished wiki before adding their own material. `Celeste` is included as a tutorial-style sample generated by the current Spec-Based Agent Pool pipeline.
...
- `Source Coverage Backfill`: migrate existing game pages to the source-agent metadata model without rewriting their analysis text
...
## Upgrade Source Coverage Metadata

If older game pages were created before the Spec-Based Agent Pool existed, run:

```bash
npm run skill -- source-coverage-backfill
```

This keeps existing analysis text intact and backfills:
...
```bash
npm run skill -- source-coverage-backfill --path wiki/games/hades.md
```
